...
unsigned int	compute_shading(t_scene scene, t_ray ray, t_object *object)
{
	...
	temp = light_gets_blocked(scene, object, intersection_point);
	if (temp == -1)
	{
		write(1, "LGB \n", 5);
		return (0x800000);
	...
	// 	return (0x000000); // dit kan misschien met pointer van objects vergelijken
